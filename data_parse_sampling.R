...
max_dna_length <- data$`DNA/RNA` %>% sapply(.,nchar) %>% max
max_pro_length <- data$Protein %>% sapply(.,nchar) %>% max
...
filled_length <- function(x, max_len){
  c(x %>% strsplit(.,split = "") %>% unlist(),rep("@",max_len-nchar(x))) %>% as.factor()%>% as.numeric()
}

protein_type <- data$Protein %>% strsplit(.,split = "") %>% unlist %>% unique() %>% c("@",.) %>% as.factor()
protein_code <- data$Protein %>% strsplit(.,split = "") %>% unlist %>% unique() %>% c("@",.) %>% as.factor() %>% as.numeric()
DNA_type <- data$`DNA/RNA` %>% strsplit(.,split = "") %>% unlist %>% unique() %>% c("@",.) %>% as.factor()
DNA_code <- data$`DNA/RNA` %>% strsplit(.,split = "") %>% unlist %>% unique() %>% c("@",.) %>% as.factor() %>% as.numeric()


# col = position 
# row = samples

DNA_array <- data$`DNA/RNA` %>% sapply(.,filled_length, max_len = max_dna_length) %>% t()
Protein_array <- data$Protein %>% sapply(.,filled_length, max_len = max_pro_length) %>% t()
y_data <- data$`Class Label` %>% sapply(.,function(x){if(x == "negative"){0} else {1} }) %>% as.integer()
...
which_0_train <- which_0[-sample_num ]

which_1_test_nerver_seen <- which_1_test[1:50]
which_0_test_nerver_seen <- c(which_0_test[1:50],which_0_unselect)
...
cat( "train = " , length(which_1_train)*2  ,"  test = " , length(which_0_test)*2 , "  never_seen = " , length(which_1_test_nerver_seen)*2  )

# 
# sample(which_0_unselect, 10)
# 
# 
# # tmp_1024_unused <- which_0_all[!which_0_all %in% c(which_0_test,which_0_train,which_0_test_nerver_seen)]
# # tmp_2048_unused <- which_0_all[!which_0_all %in% c(which_0_test,which_0_train,which_0_test_nerver_seen)]
# tmp_4096_unused <- which_0_all[!which_0_all %in% c(which_0_test,which_0_train,which_0_test_nerver_seen)]
# 
# c(which_0_test,which_0_train,which_0_test_nerver_seen) %>% length
# 
